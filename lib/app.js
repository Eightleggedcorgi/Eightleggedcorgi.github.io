...
const redoLight = document.querySelector(".redo-button");
...
document.querySelector(".next-button").onclick = function () { //button for levels
    if (levelDone === true) {
        levelCount++;
        levelDone = false;
        nextLight.classList.add("selected");
        redoLight.classList.add("selected");
        wait = true;
        if (levelCount === 1) {
            levelOne();
        }
...
document.querySelector(".redo-button").onclick = function () { //button to hear again
    if (levelDone === false) {
        wait = true;
        redoLight.classList.remove("selected");
        setTimeout(function() {
            redoLight.classList.add("selected");
        }, 200);
        if (levelCount === 1) {
...
}); //didnt work unless I made a second one
...
//functions to prevent even more nightmarish bloat and repetition
...
function superWin() { //in case of all levels complete
    levelDone = true;
    playAudio('supaCheer.wav');
    nextLight.classList.remove("selected");
    redoLight.classList.remove("selected");
...
function levelSix() {
    falsifier();
    setTimeout(fakeGreen, 300);
    setTimeout(fakeRed, 700);
    setTimeout(fakeBlue, 1200);
    setTimeout(fakeBlue, 1800);
    setTimeout(fakeRed, 2400);
    setTimeout(fakeBlue, 2900);
    setTimeout(fakeYellow, 3400);
    setTimeout(fakeYellow, 4000);
    setTimeout(fakeBlue, 4600);
    setTimeout(fakeYellow, 5200);
    setTimeout(fakeRed, 5700);
    setTimeout(fakeRed, 6200);
    setTimeout(fakeGreen, 6700);
    setTimeout(fakeBlue, 7100);
    setTimeout(fakeRed, 7500);
    setTimeout(fakeGreen, 7900);
    setTimeout(fakeYellow, 8300);
    let i = setInterval(function () {
        if (counted6 <= target6) {
            counted6++;
            if (correctCount >= lvl6Count) {
                superWin();
                clearInterval(i);
            }
            else if (wait === true) {
...
//fake level functions for the redo button
function fakeOne() {
    falsifier();
    correctCount = 0;
    setTimeout(fakeYellow, 300);
